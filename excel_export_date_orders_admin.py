...
from admin_production_by_date_dep import options1
...
def save_db(dbpath=r'C:\RSoft\Current\Reflex Footwear.sql3', excel_path=None, csv_path=None, extension="*.sqlite", csvs=True, excels=True):
    ...
    excel_path = os.path.join(
        main_path, "Production Scores Filter.xlsx") if excel_path == None else excel_path
...
# SELECT Table
    if (excels == True and csvs == True):
        ...
        for table_name in tables:
            ...
            table = pd.read_sql_query("SELECT * from ProductionBreakdown WHERE Date='%s'" % options1, db)
