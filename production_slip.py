...
def DblClick(event):
    ...
    root2.title("Slipper Production Breakdown")
...
def production():
    ...
    with sqlite3.connect('Reflex Footwear.sql3') as conn:
        ...
        mycursor.execute("SELECT Factory,Planned,Order2,Style,Deldate,Orderqty,Clicking,Closing,Finishing,Despatch,ToShip FROM SlipProduction")
        for row in mycursor:
...
style = Style()
style.configure('R.TButton', font=('Calibri', 14, 'bold',
                                   'underline'), foreground='dodgerblue4', background='grey80')
style.configure('B.TButton', font=('Calibri', 14, 'bold',
                                   'underline'), foreground='firebrick', background='grey80')
