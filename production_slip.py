...
l1 = Label(root, text="Slipper Production Tracker", width=120, anchor=CENTER, font=["Bodoni MT", 30, "bold"], background="grey80", relief="raised").pack(side='top', ipady=10)
...
def Closing():
    ...
    def ClosingUpdate():
        ...
        with sqlite3.connect('Reflex Footwear.sql3') as conn:
            cursor3 = conn.cursor()
            cursor3.execute('UPDATE SlipProduction SET Closing=Closing-? WHERE Order2=?', [balance, code,])
            cursor3.execute('UPDATE SlipProduction SET Finishing=Finishing+? WHERE Order2=?', [balance, code,])
            cursor3.execute('UPDATE SlipProd_Balances SET Closing=Closing-? WHERE Order2=?', [balance, code, ])
...
def Despatch():
    ...
    def DespUpdate():
        ...
        with sqlite3.connect('Reflex Footwear.sql3') as conn:
            ...
            cursor3.execute('UPDATE SlipProduction SET Finishing=Finishing-? WHERE Order2=?', [balance, code, ])
            cursor3.execute('UPDATE SlipProd_Balances SET Finishing=Finishing-? WHERE Order2=?', [balance, code, ])
            cursor3.execute('UPDATE SlipProd_Balances SET Despatch=Despatch-? WHERE Order2=?', [balance, code, ])
...
def Shipped():
    ...
    def ShipUpdate():
        ...
        with sqlite3.connect('Reflex Footwear.sql3') as conn:
            cursor3 = conn.cursor()
            cursor3.execute('UPDATE SlipProduction SET ToShip=ToShip-? WHERE Order2=?', [balance, code, ])
    ...
    label_0.place(x=100, y=23)
    label_2 = Label(root3, text="Invoice No.", width=20, background="lightblue2", font=("Arial", 12, "bold"))
    label_2.place(x=20, y=100)
    entry_2 = Entry(root3, textvar=Line, background="lightblue2", font=("Arial", 12, "bold"))
    entry_2.place(x=180, y=100)
    ...
    label_4 = Label(root3, text="Truck Reg.", width=20, background="lightblue2", font=("Arial", 12, "bold"))
    label_4.place(x=20, y=180)
    entry_4 = Entry(root3, textvar=Reason, background="lightblue2", font=("Arial", 12, "bold"))
    entry_4.place(x=180, y=180)
...
def Confirm():
    ...
    def ClickArch():
        ...
        with sqlite3.connect('Reflex Footwear.sql3') as conn2:
            ...
            cursor2.execute('INSERT INTO MySlippers_Archive SELECT * FROM MySlippers WHERE OrderNo=?', [code])
            cursor2.execute('DELETE FROM MySlippers WHERE OrderNo=?', [code])
            cursor2.execute('INSERT INTO SlipProd_Archive SELECT * FROM SlipProduction WHERE Order2=?', [code])
            cursor2.execute('DELETE FROM SlipProduction WHERE Order2=?', [code])
            cursor2.execute('INSERT INTO PlanSlip_Archive SELECT * FROM PlanSlip WHERE OrderNo=?', [code])
            cursor2.execute('DELETE FROM PlanSlip WHERE OrderNo=?', [code])
            ...
            cursor2.execute('INSERT INTO SlipBal_Archive SELECT * FROM SlipProd_Balances WHERE Order2=?', [code])
            cursor2.execute('DELETE FROM SlipProd_Balances WHERE Order2=?', [code])
...
# For separate button to insert scores
"""
def tkinter1():
...
def tkinter2():
    ret = os.system('python update_closing_slip.py')
    if ret:
        production()


def tkinter3():
    ret = os.system('python update_despatch_slip.py')
    if ret:
        production()


def tkinter4():
    ret = os.system('python update_warehouse_slip.py')
    if ret:
        production()


def tkinter5():
    ret = os.system('python update_shipped_slip.py')
    if ret:
        production()

"""
def tkinter6():
    ret = os.system('python excel_export_prod_slip.py')
    if ret:
        production()
...
def tkinter8():
    ret = os.system('python slip_production_balances.py')
    if ret:
        production()
