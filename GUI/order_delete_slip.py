...
def order_delete():
    ...
    with sqlite3.connect('Reflex Footwear.sql3') as conn:
        cursor = conn.cursor()
        cursor.execute('DELETE FROM MySlippers WHERE OrderNo=?', [code])
        cursor.execute('DELETE FROM SlipProduction WHERE Order2=?', [code])
        cursor.execute('DELETE FROM PlanSlip WHERE OrderNo=?', [code])
