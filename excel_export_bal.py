...
def save_db(dbpath=r'C:\RSoft\Current\Reflex Footwear.sql3', excel_path=None, csv_path=None, extension="*.sqlite", csvs=True, excels=True):
    ...
    excel_path = os.path.join(
        main_path, "Production Scores.xlsx") if excel_path == None else excel_path
...
# SELECT Table
    if (excels == True and csvs == True):
        ...
        for table_name in tables:
            ...
            table = pd.read_sql_query(f"SELECT Date,Line,Order2,Style,Cutting,Assembly,Closing,Despatch,Shipped,Reason FROM ProductionBreakdown WHERE Date = '{today}'", db)
