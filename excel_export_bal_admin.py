...
def save_db(dbpath=r'C:\RSoft\Current\Reflex Footwear.sql3', excel_path=None, csv_path=None, extension="*.sqlite", csvs=True, excels=True):
    ...
    excel_path = os.path.join(
        main_path, "Production Balances(Admin).xlsx") if excel_path == None else excel_path
...
# SELECT Table
    if (excels == True and csvs == True):
        ...
        for table_name in tables:
            ...
            table = pd.read_sql_query("SELECT Order2,Style,DelDate,Orderqty,Clicking,Closing,Finishing,Despatch,Warehouse,ToShip,Shipped from Production_Balances ORDER BY DelDate ASC", db)
